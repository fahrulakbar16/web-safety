            ...
            <ul class="max-h-24 overflow-y-auto" data-simplebar>
                ...
                >
                    <input
                        type="radio"
                        class="text-blue-600"
                        :checked="!props.modelValue || props.modelValue === null"
                        @change.stop
                    />
                    <span class="font-medium text-gray-700 dark:text-gray-300">{{ props.label }}</span>
                </li>
                ...
                >
                    ...
                    <slot name="item" :item="item">
                        <span>{{ item[props.labelKey] || item.name }}</span>
                        <!-- default fallback -->
                    </slot>
                </li>
                <li
...
function toggleDropdown() {
    ...
    if (open.value) {
        search.value = ""; // Reset search when opening
        nextTick(() => {
...
function selectItem(item) {
    ...
    closeDropdown();
    search.value = ""; // Reset search after selection
}

function clearSelection() {
    emit("update:modelValue", null);
    closeDropdown();
    search.value = ""; // Reset search after clearing
}
...
function handleClickOutside(e) {
    ...
    const isOutsideDropdown = dropdownElement.value && !dropdownElement.value.contains(target);
...
watch(() => props.modelValue, () => {
    if (open.value) {
        // Keep dropdown open for better UX, but could close if needed
        // closeDropdown();
    }
});
...
function createTag() {
    ...
    if (!exists(localAdded.value) && !exists(props.items || [])) {
        // Emit create event for parent to handle (e.g., API call)
        emit("create", label);
        // If parent doesn't handle it, create local item as fallback
        // This will be handled by parent component
    } else {
        // If exists, just select it
        const existing = [...props.items, ...localAdded.value].find(
            (i) => (i[props.labelKey] || "").toString().toLowerCase() === label.toLowerCase()
        );
        if (existing) {
    ...
    closeDropdown();
    search.value = ""; // Reset search after creating
}
