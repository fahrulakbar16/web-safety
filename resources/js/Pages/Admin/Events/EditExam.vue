<template>
    <Head :title="`Edit Ujian - ${event.name}`" />
...
const breadcrumbs = [
    ...
    { label: `Edit Ujian: ${props.exam.name}` },
];

const form = useForm({
    name: props.exam.name || "",
    description: props.exam.description || "",
    durasi: props.exam.durasi || 60,
    jumlah_soal: props.exam.jumlah_soal || 10,
    minimal_score: props.exam.minimal_score || 70,
});
...
const submitForm = () => {
    ...
    form.transform((data) => ({
        ...data,
        _method: 'PUT',
    })).post(route("events.exams.update", [props.event.id, props.exam.id]), {
        onSuccess: () => {
