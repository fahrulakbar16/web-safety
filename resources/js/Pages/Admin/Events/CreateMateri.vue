<template>
    <Head :title="`Tambah Materi - ${event.name}`" />
            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                ...
                <div v-if="form.type === 'video'" class="space-y-1 text-sm">
                    ...
                    <div class="flex flex-col gap-2">
                        <div v-if="filePreview || videoPreview" class="flex-shrink-0">
                            <video
                                v-if="videoPreview"
                                :src="videoPreview"
                                controls
                            ...
                            ></video>
                            <p v-else-if="filePreview" class="text-xs text-gray-500">
...
const handleTypeChange = () => {
    ...
    if (form.type === 'text') {
        ...
        // Clear description when switching to video
        form.description = '';
    }
};
...
const submitForm = () => {
    ...
    form.post(route("events.materis.store", props.event.id), {
        forceFormData: true,
        onSuccess: () => {
