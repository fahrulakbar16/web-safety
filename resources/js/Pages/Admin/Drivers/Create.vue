...
// Handle create company from dropdown
const handleCreateCompany = async (companyName) => {
    try {
        const response = await window.axios.post(route("companies.quick-store"), {
            name: companyName,
        });
        ...
        const errorMessage = error.response?.data?.message || error.message || "Gagal membuat perusahaan";
        alert("Gagal membuat perusahaan: " + errorMessage);
    }
};
